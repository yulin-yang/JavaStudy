...
例如下图，我们现在要查询`select * from user where name = 'Arm' `，由于给`name`添加了二级索引，所以会先。去比对，先在索引树种找到`Lee`，比对后`Arm`在左边，一直找到`Arm`，获得其对应的主键`id=10`，然后再到聚集索引中根据`id=10`找到`Arm`的信息
...
- **非聚集索引**
  ...
  **缺点**：
  ...
  - **可能会二次查询(回表)**:这应该是非聚簇索引最大的缺点了。 当查到索引对应的指针或主键后，可能还需要根据指针或主键再到数据文件或表中查询。但只是可能，并非一定。例如在上面的例子中假如我们查询的刚好是`select name from user where name = 'Arm'`
...
3.  `slave`重做中继日志中的时间, 将改变反映它自己的数据.
...
![pic_d39e2f64.png](https://mark.cuckooing.cn/pics/pic_d39e2f64.png)

垂直分库: 以表为依据, 根据业务将不同表拆分到不同库中.

![pic_04bc3ecd.png](https://mark.cuckooing.cn/pics/pic_04bc3ecd.png)
...
![pic_777b338a.png](https://mark.cuckooing.cn/pics/pic_777b338a.png)
...
![pic_963afb68.png](https://mark.cuckooing.cn/pics/pic_963afb68.png)

水平分库: 将一个库的数据拆分到多个库中.

![pic_7f35f745.png](https://mark.cuckooing.cn/pics/pic_7f35f745.png)

![pic_9f5ada1d.png](https://mark.cuckooing.cn/pics/pic_9f5ada1d.png)
...
![pic_04d7d4b7.png](https://mark.cuckooing.cn/pics/pic_04d7d4b7.png)

水平分表: 将一个表的数据拆分到多个表中(可以在同一个库中)

![pic_eeb1a2f6.png](https://mark.cuckooing.cn/pics/pic_eeb1a2f6.png)
