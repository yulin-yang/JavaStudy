...
Redisson 提供了分布式布隆过滤器的实现，称为 `RBloomFilter`，它基于 Redis 实现，支持分布式环境下的高效判断。

1. **创建和初始化布隆过滤器**

   在使用 `RBloomFilter` 时，需要指定预计插入的元素数量和期望的误判率。Redisson 会根据这些参数自动计算位数组的大小和哈希函数的数量。

   ```java
   java复制编辑RBloomFilter<String> bloomFilter = redisson.getBloomFilter("myBloomFilter");
   bloomFilter.tryInit(10000, 0.03); // 预计插入 10000 个元素，误判率为 3%
   ```
...
3. **判断元素是否存在**
   ...
   使用 `contains` 方法判断元素是否可能存在：

   ```java
   boolean exists = bloomFilter.contains("element1");
   ```

   如果返回 `true`，则该元素可能存在；如果返回 `false`，则该元素一定不存在。
...
> **思路**：热点缓存永不过期，仅在缓存中记录一个**逻辑过期时间**，当用户访问时若逻辑过期，则**异步重建**缓存，同时**立刻返回旧值**
>
> **流程**：
>
> 1. 查询缓存并检查逻辑到期时间。
 ...
 *  写操作: 延迟双删，就是在第一次删除缓存之后，等待一个足够的时间（保证读请求都结束、事务提交完成），再删除一次缓存，确保旧值不会残留。
...
1. 先删除缓存, 还是先修改数据库?
    ...
    * 所以应该先操作数据库，再删除缓存
    ...
    >
    > 此外根据gpt说法：为什么先更新数据库，再删除缓存
    >
    > - 保证数据持久化：先确保数据库写入成功后再操作缓存，避免缓存更新成功但 DB 写入失败导致的不一致。
    > - 减少脏数据风险：删除缓存后，后续查询若命中缓存会加载到最新数据库值，不会加载旧值写回缓存中。

    ![image-20241023114104428](./Redis.assets/image-20241023114104428.png)

2. 为什么要删除两次缓存?

    > 核心原因：**第一次删除可能会被并发回写“脏数据”**。
    >
    > 场景复现：
    >
    > 1. 线程 A **更新数据库** → **删除缓存**（第一次删除）。
    > 2. 线程 B 在 A **事务未提交前**去查数据：
    >    - 发现缓存没了 → 去数据库查到**旧数据**（因为事务没提交或隔离级别影响）。
    >    - 把旧数据重新**写回缓存**。
    > 3. 线程 A 事务提交，此时数据库是新值，但缓存却被 B 刷成了旧值 → **脏数据出现**。
    >
    > 所以：
    >
    > - **第一次删除**：尽快让缓存失效，让读流量打到数据库，保证读到的是尽可能新的值。
    > - **第二次删除**：清掉第一次删除之后可能被并发写回的旧数据，保证最终一致性。

3. 为什么要延时删除?

    > 第二次删除不能立即做，因为：
    >
    > - 如果立刻删，可能还是在数据库事务提交**之前**，此时并发读依旧能查到旧值并回写。
    > - 延迟的作用就是**等事务提交完成、并发读请求处理完**，这样第二次删除才是彻底清理脏数据。
    > - DB的主从复制模式，修改完数据库后延时一会儿，等到复制结束后再删除缓存。但因为时间不确定，所以仍有读脏数据的可能。通常1~2秒是比较普遍的选择
    >
    > 延迟的时间要**大于并发读耗时 + 数据库事务耗时**。

4. 为什么是删除缓存而不是更新？
   - 操作更简单，侵入少：删除缓存只需发送一个简单的 `DEL key` 命令，操作直观、低复杂度。更新复杂缓存内容往往需要获取、反序列化、修改、再序列化，工作量更大，性能开销更高。
   - 减少并发写入冲突：在高并发场景下，同一数据可能被多线程同时更新。如果直接更新缓存，容易发生覆盖竞争、写入乱序等问题。删除缓存则能避免这一类并发问题，使缓存数据由下次访问时重新加载。
   - 缓存内容通常来源于复杂计算或关联多个表的数据聚合。在写操作频繁但读操作少的场景下，反复更新缓存代价高。删除缓存可避免不必要的运算，只有在真正访问时才重新计算并加载缓存。
...
在使用 Redis 作为分布式锁时，一个常见的问题是：客户端在获得锁之后，如果执行业务逻辑超过了锁的过期时间（TTL），锁会在业务尚未完成时自动过期，被其他客户端“抢走”，可能造成并发安全问题。为了解决这个问题，Redisson（一个流行的 Java Redis 客户端）引入了 **Watchdog** 机制——一种自动延长锁过期时间的“看门狗”机制。
...
`min-replicas-to-write 1`: 指定主节点至少要有多少个从节点在线并同步，才允许写操作；

`min-replicas-max-lag 5`: 指定从节点与主节点的最大允许延迟时间。

当条件不满足时，主节点将拒绝写操作，从而避免数据在不同主之间写入冲突。
