...
> 在linux中进程无法直接操作物理内存，所以操作系统为每个进程提供了一份虚拟内存，并提供了物理内存与虚拟内存之间的映射，称之为页表。主进程只操作虚拟内存，再通过页表关联至物理内存的实际位置，从而达到对物理内存的读和写操作。随后fork一个子进程，子进程会把页表数据进行拷贝，随后子进程对虚拟内存的操作也会同步到物理内存，这就实现了子进程和主进程的内存共享。缩短了阻塞时间
