...
![](./day07-MQ高级.assets/VohQbYwSlo8R6AxfinGcLNq6nld.png)

总结如下：

* 当消息投递到MQ，但是路由失败时，通过**Publisher Return**返回异常信息，同时返回ack的确认信息，代表投递成功。（确实到达了队列，但是代码可能有问题导致路由失败，需要通过日志找原因）
...
> `ReturnCallback` 是 **Spring AMQP**（RabbitMQ 的 Spring 封装）中的一个**回调接口**，用于在 **消息无法被路由到队列时**触发回调。当消息不能被正确路由到目标队列时，`ReturnCallback` 接口用于通知生产者处理这个“路由失败的消息”。

每个`RabbitTemplate`只能配置一个`ReturnCallback`，因此我们可以在配置类中统一设置。我们在publisher模块定义一个配置类：

![](./day07-MQ高级.assets/VtLLb0dSFo6XOdx1EEiczVvbnvc.png)
...
> `ConfirmCallback` 是 Spring AMQP（RabbitMQ 的 Spring 封装）中非常重要的一个**回调接口**，用于实现 **消息从生产者可靠投递到 RabbitMQ 交换机的确认机制**。

由于每个消息发送时的处理逻辑不一定相同，因此ConfirmCallback需要在每次发消息时定义。具体来说，是在调用RabbitTemplate中的convertAndSend方法时，多传递一个参数：

![](./day07-MQ高级.assets/E2IcbQ9bQoCDTVx8m8GciifXnKe.png)
...
![](./day07-MQ高级.assets/TR81bF8E3ocm72xkaqHcuN9Nnph.png)
...
![](./day07-MQ高级.assets/YmF2bo8C8ot7CexZXopcsIyTnYc.png)
...
为了提升性能，默认情况下MQ的数据都是在**内存存储**的临时数据，这样会导致两个问题：

- 一旦MQ宕机，内存中的所有消息都会丢失
- 内存空间有限，当消费者故障或者处理消息很慢时，会导致消息积压，引发MQ阻塞
...
![](./day07-MQ高级.assets/NPr8bcUqkoNVgIxP7ePct6thn4g.png)
...
![](./day07-MQ高级.assets/CxrxbP3awotxmkxSndlcE7iKnvd.png)
...
![](./day07-MQ高级.assets/image.png)

> **说明**：在开启持久化机制以后，如果同时还开启了生产者确认，那么MQ会在消息持久化以后才发送ACK回执，进一步确保消息的可靠性。
>
> 不过出于性能考虑，为了减少IO次数，发送到MQ的消息并不是逐条持久化到数据库的，而是每隔一段时间批量持久化。一般间隔在100毫秒左右，这就会导致ACK有一定的延迟，因此建议生产者确认全部采用异步方式。

在课程中进行了一个测试，通过向MQ发送100w条消息

- 当消息非持久化的时候，会将所有消息写入内存，当内存达到上限后，随后慢慢写到磁盘（page out就是写出到磁盘），写入到磁盘的过程中消息会阻塞
  - ![image-20250628174416922](./day07-MQ高级.assets/image-20250628174416922.png)
- 当消息持久化的时候，每写入一条消息都会进行一次持久化，IO一直处于一个很高的阶段，不会出现阻塞和断点
  - ![image-20250628174520063](./day07-MQ高级.assets/image-20250628174520063.png)
...
![](./day07-MQ高级.assets/D7mmbzVySoa0VzxTQ6Ucbpl2nKb.png)
...
![](./day07-MQ高级.assets/EBkmbWPYIo1x2KxnEXUcWHcHn0U.png)
...
![](./day07-MQ高级.assets/AFUlbn4sMoqeX6xbldFcQPbYnXc.png)

- LazyQueue模式下队列的并发处理更高

![image-20250628175224818](./day07-MQ高级.assets/image-20250628175224818.png)
...
spring:
  rabbitmq:
    listener:
      simple:
        acknowledge-mode: none # none、manual、auto
```
...
![](./day07-MQ高级.assets/MaLabMxNNoh4gMxwHDxc4MxTnoh.png)

放行以后，由于抛出的是**消息转换异常**，因此Spring会自动返回`reject`，所以消息依然会被删除：

![](./day07-MQ高级.assets/OuJLbuT9eowqpaxI7TNcimyinZc.png)
...
![](./day07-MQ高级.assets/UkKwbmXVpoyeUyxxxxncpY5NnMg.png)

放行以后，由于抛出的是业务异常，所以Spring返回`ack`，最终消息恢复至`Ready`状态，并且没有被RabbitMQ删除：

![](./day07-MQ高级.assets/K8qRb41Uso5Riyx95aLcGAYQnCe.png)

当我们把配置改为`auto`时，消息处理失败后，会回到RabbitMQ，并重新投递到消费者。直到成功返回ack
...
![](./day07-MQ高级.assets/B2SpbJ5Rxo78AQxuyXQcauXsn4b.png)
...
![image-20250628183430252](./day07-MQ高级.assets/image-20250628183430252.png)
...
![image-20250628183941931](./day07-MQ高级.assets/image-20250628183941931.png)
...
![image-20250628184457512](./day07-MQ高级.assets/image-20250628184457512.png)
...
![](./day07-MQ高级.assets/diagram-1.png)
...
![](./day07-MQ高级.assets/DU6Bbv9ELogXMVxjEYZcS4C4nae.png)



假如我们现在发送一条消息到`ttl.fanout`，RoutingKey为blue，并设置消息的**有效期**为5000毫秒：

![](./day07-MQ高级.assets/IGCWbzpEKo3loaxCJWkcQukZnme.png)
...
![](./day07-MQ高级.assets/JLSsbX5RyoZCKTxCxhGcksRynFb.png)

死信被再次投递到死信交换机`hmall.direct`，并沿用之前的RoutingKey，也就是`blue`：

![](./day07-MQ高级.assets/RVgSbj8otoQtKYxsd4jc2OdFnOb.png)
...
![](./day07-MQ高级.assets/KZx0bBSh7ofh4jxA0lTcTlL1nic.png)
...
![](./day07-MQ高级.assets/KqSKbHiB5oHaCPxSVG5cBt00nac.png)
...
![](./day07-MQ高级.assets/TZdFbdnG8oJ9aSxedA9cjII6n0f.png)
...
![](./day07-MQ高级.assets/GHBGbT2WLoreOwxwyT5cPu1anOf.png)
...
![](./day07-MQ高级.assets/U4aPbhArAovCXyxs3iRcH6IUnAc.png)
