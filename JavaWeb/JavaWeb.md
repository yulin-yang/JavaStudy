...
在IDE的启动页面选择自定义修改maven的仓库设置，不要在项目中更改，项目中更改默认只对当前项目生效

![1678450168050](JavaWeb.assets/1678450168050.png)

![1678450280159](JavaWeb.assets/1678450280159.png)
...
3. 等待项目初始化完毕
   ...
   （如果在创建的时候报错：'pom.xml' already exists in VFS，则说明创建了两个项目名称一样的maven项目，虽然删除了一个但是仍存在于电脑中，解决方法是将回收站中已经删除的maven项目也删除掉）
...
6. 到这里，Maven在IDEA中的配置和使用就OK了!但是以上的创建方法并不是最好的，因为我们使用了模板来生成。

### 5.7、创建一个普通的Maven项目（最干净的maven项目）
...
![1678441979351](JavaWeb.assets/1678441979351.png)
...
target目录是java编译后生成class文件存放的目录
...
百度搜索“maven仓库”，然后点击需要的依赖，赋值到pom.xml中，maven会自动下载并导入这个jar包

![1603616934275](C:\Users\12620\AppData\Local\Temp\1603616934275.png)



maven的高级之处在于导入一个包以后会自动帮你导入该包依赖的其他jar包



**一般不推荐使用maven模板创建maven项目，一般都是创建一个普通的maven项目然后手到那个变为web项目，添加java文件夹和resource文件夹并设置文件夹功能。**



maven由于他的约定大于配置（必须按照maven的规则建文件夹并在指定的文件夹中写东西），我们之后可以能遇到我们写的配置文件，无法被导出或者生效的问题，解决方案：
...
## 15、DAO（DATA Accsee Object数据存取对象）



如果没有dao，多个地方都要做CEUD操作时，重复的代码会很多



![1603786997289](JavaWeb.assets\1603786997289.png)
...
1. 创建一个domain（含有setter()和getter()方法的Javabean）
...
4. 测试
   ...
   ```java
   @Test(需要导入Junit，选择“import 'Test'(org.junit)”)
   public void save() { 
       Student stu=new Student(); 
       stu.setAge(20);
       stu.setName("liusi"); 
       IStudentDao dao=new StudentDaoImpl();  //注意这里new的是一个实现类，但是创建的是dao对象
       dao.save(stu);
...
**baseServlet是一个java类，可调用继承此类中的任意方法，所有的其他servlet都继承BaseServlet****

在jsp中单击某个按钮后，通过超链接跳转至相应的servlet（该servlet一定继承baseServlet）

?method=add  表示的是参数

userServlet中的所有方法都要是public，否则无法调用。

```java
<a href="userServlet?method=addArticle">增加</a>
<a href="userServlet?method=getAllArticles">查询</a>
```

当跳转到studentServlet时，会先去寻找post/get/service方法，但是在studentServlet中没有，于是到baseServlet中去，执行baseServlet中的service方法。
...
public class BaseServlet extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			Method method = clazz1.getMethod(action, HttpServletRequest.class, HttpServletResponse.class);
			if (method != null) {
                //执行方法并获得方法返回值
...
public class UserServlet extends BaseServlet {
	...
	// 获取所有的文章
	public String getAllArticles(HttpServletRequest request, HttpServletResponse response)
			throws ServletException, IOException {
		request.setCharacterEncoding("utf-8");

		// 用一个数组来接受service层传递过来的allArticle
		List<Article> allArticles = new ArrayList<>();
		UserService userService = new UserService();
		Object attribute = request.getSession().getAttribute("admin");
		try {
			allArticles = userService.getAllArticles();
			// 对集合进行反转
			Collections.reverse(allArticles);
			// 保存到request域当中
			request.setAttribute("allArticles", allArticles);
			// 从session域当中取管理员，如果是管理员则跳转到admin/main.jsp,如果是null则说明是用户则跳转到user/use_main.jsp
			if (attribute == null) {
	...
	public String addArticle(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			// 跳转回主页面
			return "/UserServlet?action=getAllArticles";
		} catch (Exception e) {
	...
	// 根据id查询一篇文章的详细信息
	public String CheckDetails(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		System.out.println("writer="+writer+"     username="+username);
		UserService userService = new UserService();
		try {
			Article article = userService.CheckDetails(id);
			// 保存到session域当中
			request.getSession().setAttribute("article", article);
			// 如果是自己的文章或者当前登录用户为管理员，则跳转至change.jsp页面可以对文章进行修改和保存，如果不是则跳转到details页面只可以查看当前文章
	...
	// 修改当前用户的个人信息
	public String ChangeSelf(HttpServletRequest request, HttpServletResponse response) throws Exception {
		request.setCharacterEncoding("utf-8");
		UserService userService = new UserService();
		User u = new User();
		Map<String, String[]> parameterMap = request.getParameterMap();
		...
		userService.ChangeSelf(u);
		return "user/self.jsp";
	}

	// 查看自己发布的文章
	public String SelfArticles(HttpServletRequest request, HttpServletResponse response) throws Exception {
		List<Article> selfArticles = new ArrayList<>();

		String writer = ((User) request.getSession().getAttribute("user")).getUsername();
		UserService userService = new UserService();
		try {
			selfArticles = userService.SelfArticles(writer);
			// 对集合进行反转
			Collections.reverse(selfArticles);
			// 保存到request域当中
			request.setAttribute("selfArticles", selfArticles);
			return "user/self_articles.jsp";
	...
	// 删除自己发布的文章
	public String deleArticles(HttpServletRequest request, HttpServletResponse response) {
		String id = request.getParameter("id");
		//从session域当中获取管理员，判断是否为空，若否，则说明当前登录用户为管理员
		Object attribute = request.getSession().getAttribute("admin");

		UserService userService = new UserService();
		try {
			userService.deleArticle(id);
			// 如果当前用户为管理员的话跳转至所有文章界面，如果为普通用户则跳转到自己的文章页面
			if (attribute != null) {
				return "UserServlet?action=getAllArticles";
			} else {
				return "UserServlet?action=SelfArticles";
			}
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
	...
	// 根据id修改已经发布的文章
	public String changeArticle(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			BeanUtils.populate(article, parameterMap);
			String writer = ((Article) request.getSession().getAttribute("article")).getWriter();
			article.setWriter(writer);
			System.out.println(article);
			UserService userService = new UserService();
			userService.changeArticle(article);
			request.getSession().setAttribute("article", article);
			return "user/change.jsp";
		} catch (Exception e) {
			e.printStackTrace();
	...
	//收藏文章
	public String collArticle(HttpServletRequest request, HttpServletResponse response) {
		...
		UserService userService = new UserService();
		try {
			userService.collArticle(uid, cid);
			//返回到所有文章界面
			return "/UserServlet?action=getAllArticles";
		} catch (Exception e) {
			// TODO Auto-generated catch block
	...
	//获取所有当前用户收藏的文章
	public String myCollArticles(HttpServletRequest request, HttpServletResponse response) {
		//获取当前用户id
		String uid = ((User)request.getSession().getAttribute("user")).getId();
		List<CollArticle> myCollArticles=new ArrayList<CollArticle>();
		UserService userService = new UserService();
		
		try {        
			//先到collarticle当中取出所有当前用户收藏的文章的id
			myCollArticles=userService.myCollArticles(uid);
			
			//遍历myCollArticles,到article数据表当中取出所有与myCollArticles编号对应的文章	，添加到articles集合当中
			List<Article> articles=new ArrayList<Article>();
			
			for(int i=0;i<myCollArticles.size();i++) {				
				Article article=new Article();
				article=userService.getArticleWithId(myCollArticles.get(i).getCid());
				articles.add(article);			
			}
			//将articles保存到session域当中
			request.getSession().setAttribute("articles", articles);
			
			//跳转到界面显示所有已经收藏的文章		
			return "/user/mycollArticle.jsp";
	...
	public String search(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		UserService userService = new UserService();
		List<Article> searchArticles = userService.search(key);
		//保存到session域当中
		request.getSession().setAttribute("searchArticles", searchArticles);
		//跳转到页面
		return "/user/searchArticles.jsp";
